...
(describe "the game logic"
  ...
  (context "when determining a winner"
    ...
    (it "generates heuristic values for players"
      (let [x-value (game/valuer-for :x)
            o-value (game/valuer-for :o)]
        (should-be zero? (x-value @state))
        (should-be zero? (o-value @state))
        (should-be zero? (x-value @state-draw))
        (should-be zero? (o-value @state-draw))
        (should-be pos? (x-value @state-x-row))
        (should-be neg? (o-value @state-x-row))
        (should-be pos? (x-value @state-x-col))
        (should-be neg? (o-value @state-x-col))
        (should-be pos? (x-value @state-x-diag))
        (should-be neg? (o-value @state-x-diag))
        (should-be neg? (x-value @state-o-row))
        (should-be pos? (o-value @state-o-row))))))
